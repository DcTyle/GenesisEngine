...
bool GE_load_lane_thresholds_from_file(const std::string& path_utf8, GE_AllLaneThresholds* out) {
    ...
    while (std::fgets(line, sizeof(line), f)) {
        ...
        for (const auto& t : toks) {
            std::string_view ksv, vsv;
            if (!ew::ew_split_kv_token_ascii(std::string_view(t), ksv, vsv)) continue;
            const std::string k(ksv);
            const std::string v(vsv);
            if (k=="lane") {
                if (has_cur) { out->lanes.push_back(cur); cur = GE_LaneThresholdSchedule(); }
                has_cur=true;
                uint32_t u=0;
                if (!ew::ew_parse_u32_ascii(std::string_view(v), u)) { std::fclose(f); return false; }
        ...
        for (const auto& t: toks) {
            ...
            const std::string k(ksv);
            const std::string v(vsv);
            if (k=="epoch") { uint32_t u=0; if(!ew::ew_parse_u32_ascii(std::string_view(v), u)) { std::fclose(f); return false; } epoch=u; has_epoch=true; }
