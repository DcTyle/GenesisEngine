...
set CONFIGURE_LOG=%LOGDIR%\build_editor_win64_configure.log
set BUILD_LOG=%LOGDIR%\build_editor_win64_build.log
...
>> "%BUILD_LOG%" echo [GENESIS] OK: genesis_app built
...
python "%NOTEBOOK_LOGGER%" capture --repo-root "%ROOT%" --event "build_editor_win64" --status "%NOTEBOOK_STATUS%" --script "scripts/build_editor_win64.bat" --log "%CONFIGURE_LOG%" --log "%BUILD_LOG%" --also-known-logs >nul 2>nul
exit /b 0
