...
static inline uint64_t mix_u64(uint64_t x) {
    // SplitMix64-style mixing (deterministic, fast). This is for reproducibility signatures,
...
static inline void fold_id9(uint64_t& h, const EwId9& id9) {
    for (int i = 0; i < 9; ++i) fold(h, (uint64_t)id9.u32[i]);
}
...
uint64_t ge_compute_state_signature_9d(const SubstrateManager* sm) {
    ...
    for (size_t i = 0; i < sample_n; ++i) {
        ...
        if (a.kind_u32 == EW_ANCHOR_KIND_SPECTRAL_FIELD) {
            ...
            fold_id9(h, ss.leakage_id9);
            // Temporal coupling measurement lanes.
            fold_id9(h, ss.temporal_residual.intent_id9);
            fold_id9(h, ss.temporal_residual.measured_id9);
            ...
            fold_id9(h, ss.pulse_measured.pulse_intent_id9);
            fold_id9(h, ss.pulse_measured.pulse_measured_id9);
        ...
        if (a.kind_u32 == EW_ANCHOR_KIND_VOXEL_COUPLING) {
            ...
            fold_id9(h, vs.temporal_residual.intent_id9);
            fold_id9(h, vs.temporal_residual.measured_id9);
            fold(h, (uint64_t)vs.temporal_residual.residual_q32_32);
            fold(h, (uint64_t)vs.temporal_residual.residual_norm_q15);
            fold_id9(h, vs.pulse_measured.pulse_intent_id9);
            fold_id9(h, vs.pulse_measured.pulse_measured_id9);
