...
void genesis::LearningAutomation::emit_lane_status_lines(SubstrateManager* sm) {
    if (!sm) return;
    sm->emit_ui_line(std::string("AI_LANES stage=") + std::to_string((unsigned long long)sm->learning_curriculum_stage_u32));
}
