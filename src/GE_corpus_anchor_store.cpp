...
EwId9 GE_anchor_id9_from_provenance_and_carrier(uint8_t lane_u8,
    ...
    // Pack lane + seq + chunk_offset_q into u32[8], then fold in residual carrier bits.
...
bool GE_CorpusAnchorStore::load_from_file(const std::string& path_utf8) {
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        int32_t f_code_i32 = 0;
        int32_t a_code_i32 = 0;
        int32_t v_code_i32 = 0;
        int32_t i_code_i32 = 0;
        if (!rd_i32(f_code_i32) || !rd_i32(a_code_i32) || !rd_i32(v_code_i32) || !rd_i32(i_code_i32)) return false;
        r.sc4.f_code = f_code_i32;
        r.sc4.a_code = (uint16_t)(uint32_t)a_code_i32;
        r.sc4.v_code = (uint16_t)(uint32_t)v_code_i32;
        r.sc4.i_code = (uint16_t)(uint32_t)i_code_i32;
        uint8_t buf8[8];
