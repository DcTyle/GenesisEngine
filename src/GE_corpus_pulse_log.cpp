...
bool GE_CorpusPulseLog::load_from_file(const std::string& path_utf8) {
    ...
    for (uint32_t i=0;i<count;i++){
        ...
        uint32_t sc4_f_u32 = 0u;
        uint32_t sc4_a_u32 = 0u;
        uint32_t sc4_v_u32 = 0u;
        uint32_t sc4_i_u32 = 0u;
        ok = ok && ge_read_u32(f,&sc4_f_u32);
        ok = ok && ge_read_u32(f,&sc4_a_u32);
        ok = ok && ge_read_u32(f,&sc4_v_u32);
        ok = ok && ge_read_u32(f,&sc4_i_u32);
        r.sc4.f_code = (int32_t)sc4_f_u32;
        r.sc4.a_code = (uint16_t)sc4_a_u32;
        r.sc4.v_code = (uint16_t)sc4_v_u32;
        r.sc4.i_code = (uint16_t)sc4_i_u32;
