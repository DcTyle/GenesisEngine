...
static std::vector<uint64_t> run_trace(uint64_t seed, std::vector<int16_t>* out_res_at_ticks) {
    ...
    for (uint32_t t = 0u; t < 40u; ++t) {
        ...
        const uint64_t signature = ge_compute_state_signature_9d(&sm);
        trace.push_back(signature);
...
int main() {
    ...
    if (a.size() != b.size()) {
        std::cerr << "FAIL: signature length mismatch\n";
        return 1;
    }
    for (size_t i = 0; i < a.size(); ++i) {
        if (a[i] != b[i]) {
            std::cerr << "FAIL: signature mismatch at tick " << i << "\n";
            return 1;
        }
    }
    ...
    std::cout << "PASS: deterministic signature stream + OP results\n";
    return 0;
}
