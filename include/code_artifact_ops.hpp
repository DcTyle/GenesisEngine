...
// into substrate inspector fields. A later apply-projection step may materialize committed
...
enum EwPatchModeU16 : uint16_t {
    ...
    EW_PATCH_REPLACE_EXACT = 5,
    EW_PATCH_DELETE_EXACT = 6,
    EW_PATCH_INSERT_BEFORE_EXACT = 7,
    EW_PATCH_INSERT_AFTER_EXACT = 8,
};

struct EwPatchSpec {
    uint16_t mode_u16 = EW_PATCH_APPEND_EOF;
    std::string anchor_a; // coherence anchor name or exact-text needle
    std::string anchor_b; // coherence end anchor when applicable
...
// Emit an apply-target hint artifact (does not write filesystem).
