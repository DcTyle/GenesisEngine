...
class Qubit:
    ...
    def serialize_state(self, use_tokens: bool = True) -> Dict[str, str]:
        ...
        # Optional high-precision path (env USE_HIGH_PRECISION=1, HP_DPS for digits)
        use_hp = getattr(hamiltonian_eq, "_hp_enabled", lambda: False)()
        if use_hp and hasattr(hamiltonian_eq, "evolve_state_two_level_hp"):
            next_state = hamiltonian_eq.evolve_state_two_level_hp(
                self.state,
                float(dt_s),
                field_strength=self.meta["field_strength"],
                temperature_k=self.meta["temperature_k"],
    ...
    def _commit(self) -> None:
        ...
        operator_chain = getattr(self, "operator_chain", None)

        delta_evolution_vector = getattr(self, "delta_evolution_vector", None)

        temporal_trace_sha = hashlib.sha256(str(operator_chain).encode("ascii")).hexdigest() if operator_chain is not None else None

        if operator_chain is None or delta_evolution_vector is None or temporal_trace_sha is None:

            raise RuntimeError("Missing provenance objects; cannot commit Qubit state vector.")
        ...
        utils.store(self.vsd_key_prefix() + "/meta", meta)
    ...
    def restore_from_vsd(self) -> None:
        ...
        meta = utils.get(self.vsd_key_prefix() + "/meta", {})

        operator_chain = meta.get("operator_chain", None)

        delta_evolution_vector = meta.get("delta_evolution_vector", None)

        temporal_trace_sha = meta.get("temporal_trace_sha", None)

        expected_sha = hashlib.sha256(str(operator_chain).encode("ascii")).hexdigest() if operator_chain is not None else None
        ...
        if operator_chain is None or delta_evolution_vector is None or temporal_trace_sha is None:
            ...
            raise RuntimeError("Missing provenance objects; cannot restore Qubit state vector.")

        if temporal_trace_sha != expected_sha:

            raise RuntimeError("Temporal trace hash mismatch; provenance validation failed.")

        self.operator_chain = operator_chain

        self.delta_evolution_vector = delta_evolution_vector
